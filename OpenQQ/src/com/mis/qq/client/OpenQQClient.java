...
import com.mis.qq.constant.ApiURL;
 ...
 * @date 2015/12/18.
 */
public class OpenQQClient implements Closeable {

    //日志
    private static final Logger LOGGER = Logger.getLogger(OpenQQClient.class);
    ...
    public OpenQQClient(final MessageCallback callback) {
    ...
    private void pollMessage(MessageCallback callback) {
        ...
        for (int i = 0; array != null && i < array.size(); i++) {
            ...
            if ("message".equals(type)) {
                ...
                GroupMessage ms = new GroupMessage(message.getJSONObject("value"));
                if(ms.getContent().equals("直播")){
                	Control control = new Control();
                	StringBuffer online = new StringBuffer();
                	StringBuffer offline = new StringBuffer();
                	List<Presenter> lisPresenter = control.getPresenterLis();
                	int sum =0;
                	...
                	if(sum>0){
                		...
                		online.append("正在直播");
                		offline.append("已经离线");
                		boolean success = sendMessageToGroup(ms.getGroupId(),online.toString()+" -- "+offline.toString());
                		if(!success){
                			sendMessageToGroup(ms.getGroupId(),online.toString()+" -- "+offline.toString());
                		}
                	}else{
                		
                		boolean success = sendMessageToGroup(ms.getGroupId(),"所有关注主播均已离线");
                		if(!success){
                			sendMessageToGroup(ms.getGroupId(),"所有关注主播均已离线");
                		}
                	}
                }
//                sendMessageToGroup(ms.getGroupId(),ms.getContent());
    ...
    private static JSONObject getResponseJson(Response<String> response) {
        if (response.getStatusCode() != 200) {
        	System.out.println("请求失败，试试能不能继续接收");
        	JSONObject json = JSON.parseObject(response.getBody());
        	return json;
